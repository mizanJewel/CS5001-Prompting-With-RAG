...
- Replaced the nested list comprehension with a more readable double loop for matrix initialization
- Renamed variables to be more descriptive (e.g., `row_st` to `row_start`)
- Maintained the exact same logic and behavior as the original implementation
- Preserved all function names and argument handling
- Ensured no value loss during any operations
- Kept the same control flow and loop structures
- Maintained the same edge case handling for n <= 0
- Preserved the exact same matrix filling pattern
