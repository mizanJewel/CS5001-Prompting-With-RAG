...
```bash
ollama pull ministral-3:8b-cloud
OLLAMA_MODEL=lministral-3:8b-cloud OLLAMA_TEMPERATURE=0.0 python3 zero_shot_refactor.py
```
...
```bash
cd dataset/outputs
python3 -m pytest -q 2>&1 | tee explanations/pytest_error_log.txt
```
...
```bash
python3 rag/rag_explain_chat.py
```
...
* Save logs for any failures into `dataset/outputs/error_logs/` (`python3 -m pytest -q 2>&1 | tee explanations/pytest_error_log.txt`).
